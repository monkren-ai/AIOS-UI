    ...
    return (
      <div
        ...
        className={cn(
          ...
          `absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-[12px] items-center ${className || ''}`.trim(),
        )}
      ...
      >
        <div className="h-[69px] widget-relative w-[50px]" data-name="image 5">
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <img
              alt=""
              className="absolute h-[213.23%] left-[-58.7%] max-w-none top-[-57.55%] w-[217.39%]"
              src={imgImage5}
            />
          </div>
        </div>
...
export const AIOSEar = React.forwardRef<HTMLDivElement, WidgetSubProps>(
  ({ theme, size, className, 'aria-label': ariaLabel, style, ...props }, ref) => {
    return (
      <div
        ref={ref}
        ...
        data-size={dataAttr(size)}
        {...props}
        data-name="AIOS Ear 2"
        aria-label={ariaLabel || 'AIOS Ear 2'}
      >
        <div
        ...
        />
        <Image />
        <p className=" absolute bottom-[16px] left-1/2 -translate-x-1/2 widget-text widget-text--dotmatrix widget-text--16 widget-text--grey2 widget-text--center widget-text--uppercase widget-text--nowrap widget-leading-19">
          CONNECT
        </p>
      </div>
    )
