    ...
    return (
      <div
        ...
        className={cn(
          ...
          `-translate-x-1/2 -translate-y-1/2 absolute h-[26.5px] left-1/2 top-[calc(50%+0.25px)] w-[26px] ${className || ''}`.trim(),
        )}
          ...
          >
            <g id="Arrow">
              <path
                d="M1 14L14 1L27 14"
                id="Vector 8"
                stroke="var(--widget-primary-dark)"
                strokeLinecap="round"
  ...
  ({ theme, size, className, 'aria-label': ariaLabel, style, ...props }, ref) => {
    return (
      <div
        ...
        className={cn(
          widgetSubVariants({ theme, size }),
          `widget-relative widget-shrink-0 size-[32px] ${className || ''}`.trim(),
        )}
        data-theme={dataAttr(theme)}
        data-size={dataAttr(size)}
        {...props}
  ...
  ({ theme, size, className, 'aria-label': ariaLabel, style, minutes, ...props }, ref) => {
    return (
      <div
        ...
        className={cn(
          widgetSubVariants({ theme, size }),
          ` content-stretch flex flex-col gap-[2px] items-start widget-relative widget-shrink-0 widget-text widget-text--nowrap ${className || ''}`.trim(),
        )}
        data-theme={dataAttr(theme)}
        data-size={dataAttr(size)}
        {...props}
        data-name="Limit Count"
        aria-label={ariaLabel || 'Limit Count'}
      >
        <p
          className="widget-leading-0 widget-relative widget-shrink-0 widget-sr text-white"
          style={{ fontVariationSettings: "'wdth' 100" }}
        >
          <span
            className="widget-text widget-text--light widget-text--32"
            style={{ fontVariationSettings: "'wdth' 100" }}
          >
            {minutes}
          </span>
          ...
          <span
            className="widget-text widget-text--light widget-text--16"
            style={{ fontVariationSettings: "'wdth' 100" }}
          >
            MIN
          </span>
        ...
        <p
          className="widget-leading-normal widget-relative widget-shrink-0 widget-text widget-text--10 widget-text--grey2 widget-text--center widget-text--uppercase"
          style={{ fontVariationSettings: "'wdth' 100" }}
        >
          Over Limit
        </p>
  ...
  ({ theme, size, className, 'aria-label': ariaLabel, style, minutes = 30, ...props }, ref) => {
    if (theme === 'dark') {
      return (
        <div
          ...
          aria-label={ariaLabel || 'Over Limit'}
        >
    ...
    return (
      <div
        ...
        style={style}
        className={cn(
          widgetSubVariants({ theme: theme === 'light' ? 'light' : 'dark', size }),
          `content-stretch flex flex-col gap-[2px] items-start widget-relative widget-shrink-0 widget-text widget-text--center widget-opacity-70 ${className || ''}`.trim(),
        )}
        data-theme={dataAttr(theme)}
        data-size={dataAttr(size)}
        data-variant={dataAttr(theme)}
...
export const Overlimit = React.forwardRef<HTMLDivElement, OverLimitProps>(
  (
    { size, className, 'aria-label': ariaLabel, style, minutes = 30, theme = 'dark', ...props },
    ref,
  ) => {
    return (
      <div
        ...
        {...props}
        data-name="Overlimit"
        aria-label={ariaLabel || 'Overlimit'}
      >
        <div className="flex flex-col justify-center size-full">
