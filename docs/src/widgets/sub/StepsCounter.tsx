    ...
    return (
      <div
        ...
        className={cn(
          ...
          `content-stretch flex flex-col gap-[2px] items-start widget-relative widget-shrink-0 ${className || ''}`.trim(),
        )}
        data-theme={dataAttr(theme)}
        data-size={dataAttr(size)}
        {...props}
        data-name="Steps Count"
        aria-label={ariaLabel || 'Steps Count'}
      >
        <p
          className="widget-text widget-text--10 widget-text--grey widget-text--uppercase"
  ...
  ({ theme, size, className, 'aria-label': ariaLabel, style, ...props }, ref) => {
    return (
      <div
        ...
        data-size={dataAttr(size)}
        {...props}
        data-name="Steps Counter"
        aria-label={ariaLabel || 'Steps Counter'}
      >
