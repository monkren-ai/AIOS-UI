...
function generateReply(input: string): string {
  ...
  if (trimmed.length < 6) return `你发送了："${trimmed}"。可以多说一点，我会继续思考。`
...
export function AIPocPage() {
  ...
  const handleCancel = React.useCallback(() => {
    cancelledRef.current = true
    clearTimers()
    updateActiveConversation((conversation) => ({ ...conversation, loading: false }))
  ...
  const promptTemplates: PromptItem[] = React.useMemo(
    () => [
      ...
      },
      {
        ...
        description: t('生成可运行的 React 代码', 'Generate runnable React code'),
      },
      {
        key: 'design',
        title: t('解释设计系统', 'Explain the design system'),
        description: t('了解令牌、颜色与布局原则', 'Learn about tokens, colors, and layout'),
      },
    ],
    [t],
  )
  ...
  return (
    <main className="ai-poc-page">
      ...
      <div className="ai-poc-layout ai-poc-layout--three-col">
        <nav className="ai-poc-conversations" aria-label={t('会话列表', 'Conversations')}>
          <Conversations
        ...
        <section className="ai-poc-chat" aria-label={t('对话', 'Conversation')}>
          <div className="ai-poc-chat__scroll">
            {isEmpty ? (
              <Welcome
                ...
                extra={t('切换左侧会话可体验多会话管理。', 'Switch conversations on the left.')}
              />
            ) : (
              <BubbleList
                items={bubbleItems}
          ...
          <div className="ai-poc-input">
            <Sender
              ...
              placeholder={t('输入消息，按 Enter 发送…', 'Type a message and press Enter…')}
              onChange={setDraft}
