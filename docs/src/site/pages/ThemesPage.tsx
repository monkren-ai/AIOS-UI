...
function ThemePreview({ theme, active, onEnable }: { theme: ThemeDefinition; active: boolean; onEnable: () => void }) {
  const { t } = useT()
  ...
  return (
    <article className="rounded-card border border-border bg-surface p-4" aria-current={active ? 'true' : undefined}>
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="font-mono text-label uppercase tracking-widest text-foreground-subtle">
            {theme.source === 'builtin' ? t('内置', 'Built-in') : t('本地', 'Local')} · {coverage(theme)}%
          </p>
          <h3 className="mt-1 text-subheading font-medium text-foreground-display">{theme.name}</h3>
          <p className="mt-1 text-body-sm text-foreground-muted">{theme.description}</p>
        </div>
        <Button size="sm" variant={active ? 'secondary' : 'primary'} disabled={active} onClick={onEnable}>
          {active ? t('已启用', 'Active') : t('启用', 'Enable')}
        </Button>
      </div>
      <div className="mt-4 flex gap-1" aria-label={t('主题色板', 'Theme palette')}>
        {swatches.map((color, index) => <span key={`${color}-${index}`} className="h-7 flex-1 border border-border" style={{ background: color }} />)}
      </div>
      <div
        className="mt-3 rounded-card border border-border bg-background p-3 text-foreground"
      ...
      >
        ...
        <div className="mt-3 flex gap-2">
          <span className="rounded-button bg-accent px-3 py-1.5 text-label font-medium text-white">Action</span>
          <span className="rounded-button border border-border-visible px-3 py-1.5 text-label">Control</span>
        </div>
      </div>
    </article>
  )
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-10 md:px-6 md:py-14">
      <header className="max-w-3xl">
        <p className="font-mono text-label uppercase tracking-widest text-accent">AIOS UI 3.0</p>
      ...
      <section className="mt-12" aria-labelledby="current-theme">
        ...
        <div className="mt-4 grid gap-4 rounded-card border-2 border-accent bg-surface p-5 md:grid-cols-[1fr_auto]">
          <div>
            <p className="font-mono text-label uppercase tracking-widest text-accent">{t('已启用', 'Active')}</p>
      ...
      <section className="mt-12" aria-labelledby="custom-themes">
        ...
        {persistenceError && <p role="status" className="mt-4 border-s-2 border-warning ps-3 text-body-sm text-foreground">{persistenceError}</p>}
        <div
          className={cn('mt-4 grid min-h-40 place-items-center rounded-card border border-dashed p-6 text-center', dragging ? 'border-accent bg-accent-subtle' : 'border-border-visible bg-surface')}
      ...
      {result && <div className="fixed inset-0 z-modal grid place-items-center bg-overlay-heavy p-4" role="dialog" aria-modal="true" aria-labelledby="theme-review-title">
        <div className="max-h-[85vh] w-full max-w-2xl overflow-auto rounded-card border border-border-visible bg-surface p-6">
          <p className="font-mono text-label uppercase tracking-widest text-accent">{fileName}</p>
          <h2 id="theme-review-title" className="mt-2 text-heading text-foreground-display">{t('导入审查', 'Import review')}</h2>
          ...
          {result.errors.length > 0 && <div className="mt-4"><h3 className="font-medium text-error">{t('错误', 'Errors')}</h3><ul className="mt-2 list-disc ps-5 text-body-sm text-foreground">{result.errors.map((item) => <li key={item}>{item}</li>)}</ul></div>}
          ...
          {result.contrastWarnings.length > 0 && <div className="mt-4 border-s-2 border-warning ps-3"><h3 className="font-medium text-warning">{t('对比度警告', 'Contrast warnings')}</h3>{result.contrastWarnings.map((item) => <p className="mt-1 text-body-sm text-foreground" key={item}>{item}</p>)}</div>}
          <div className="mt-6 flex justify-end gap-2"><Button variant="ghost" onClick={() => setResult(null)}>{t('取消', 'Cancel')}</Button><Button disabled={!result.theme} onClick={() => void confirmImport()}>{t('加入主题目录', 'Add theme')}</Button></div>
        </div>
      </div>}
    </main>
  )
