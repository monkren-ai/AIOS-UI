...
export function getComponentPage(value: string | null): ComponentPage {
  return (
    COMPONENT_PAGES.find((page) => page.id === value) ?? COMPONENT_PAGES[0]
  );
}

export function getComponentPageByCategory(
  categoryId: string | undefined,
): ComponentPage {
  return (
    COMPONENT_PAGES.find(
      (page) => categoryId && page.categoryIds.includes(categoryId),
    ) ?? COMPONENT_PAGES[0]
  );
}
