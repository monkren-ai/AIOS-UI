 ...
 * 面向中文界面的组件名称。
 *
 * `name` 仍然是稳定的 React 导出名；中文名只负责文档导航与展示，避免把
 * 本地化文案混进 import、API 表格和代码示例。
 */
export const COMPONENT_NAME_ZH: Record<string, string> = {
  IconButton: "图标按钮",
  Chip: "筛选标签",
  AvatarGroup: "头像组",
  Icon: "图标适配器",
...
const ENTRIES: ComponentManifestEntry[] = [
  ...
  },
  {
    ...
    description: {
      zh: "强制提供可访问名称的纯图标按钮。",
      en: "An icon-only button that requires an accessible name.",
    },
  },
  {
    slug: "chip",
    ...
    description: {
      zh: "用于筛选与快速选择的可滚动标签组。",
      en: "Scrollable chips for filters and quick selections.",
    },
  },
  {
    slug: "avatar-group",
    ...
    description: {
      zh: "带重叠排列和数量折叠的头像集合。",
      en: "Overlapping avatars with an overflow count.",
    },
  },
  {
    slug: "icon",
  ...
  },
  {
    ...
    description: {
      ...
      en: "A floating assistant entry backed by AssistantPanel behavior.",
    },
  },
  {
    slug: "code-block",
    ...
    description: {
      ...
      en: "Code output with copy, line numbers, wrapping, and lazy highlighting.",
    },
  },
  {
    slug: "code-diff",
  ...
  },
  {
    ...
    status: "stable",
    description: {
      zh: "Agent 状态、计划、审批、子 Agent、终端与悬浮助手面板。",
      en: "Agent state, plans, approvals, subagents, terminal output, and an assistant panel.",
    },
  },
  {
  ...
  },
  {
    ...
    description: {
      ...
      en: "Messages, input, attachments, branches, Markdown responses, sources, and intelligent scrolling.",
    },
  },
  {
    slug: "dot-matrix",
...
export const COMPONENT_MANIFEST = ENTRIES.sort((a, b) =>
...
export function groupedComponentManifest() {
  return CATEGORIES.map((category) => ({
    category,
    entries: COMPONENT_MANIFEST.filter(
