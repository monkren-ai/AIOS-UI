...
describe("component registry", () => {
  ...
  it("uses slugs that are URL-safe and match the route param", () => {
    for (const entry of COMPONENT_MANIFEST) {
      expect(entry.slug, `${entry.name} slug`).toMatch(
        /^[a-z0-9]+(-[a-z0-9]+)*$/,
      );
    }
  });
  ...
  it("points every entry at a category that exists", () => {
    for (const entry of COMPONENT_MANIFEST) {
      expect(
        CATEGORY_BY_ID.has(entry.category),
  ...
  it("describes every entry in both languages", () => {
    for (const entry of COMPONENT_MANIFEST) {
      expect(entry.description.zh.length, `${entry.name} zh`).toBeGreaterThan(
        0,
      );
      expect(entry.description.en.length, `${entry.name} en`).toBeGreaterThan(
        0,
      );
    }
  });
  ...
  it("gives every component a Chinese display name without replacing its export name", () => {
    for (const entry of COMPONENT_MANIFEST) {
      expect(
        COMPONENT_NAME_ZH[entry.name],
        `${entry.name} Chinese name`,
      ).toBeTruthy();
  ...
  it("loads a doc whose fields agree with the manifest", async () => {
    const documented = COMPONENT_MANIFEST.filter((entry) =>
      hasComponentDoc(entry.slug),
    );
    expect(
      documented.length,
      "at least one component should be documented",
    ).toBeGreaterThan(0);

    const docs = await Promise.all(
      documented.map((entry) => loadComponentDoc(entry.slug)),
    );
  ...
  it("has no orphan entry file that the manifest forgot to list", async () => {
    ...
    for (const path of Object.keys(modules)) {
      ...
      expect(
        COMPONENT_MANIFEST_BY_SLUG.has(slug),
