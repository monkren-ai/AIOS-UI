 ...
 *
 * Packages the `nothing-design/` skill folder into a distributable archive
 * (.tar.gz by default, .zip with --zip when bsdtar -a is available).
 *
 * Usage:
 *   node scripts/download-skill.mjs           # → dist/nothing-design-skill.tar.gz
 *   node scripts/download-skill.mjs --zip     # → dist/nothing-design-skill.zip (if supported)
 *
...
const SKILL_ROOT = resolve(__dirname, '..')            // nothing-design-skill/
const SKILL_FOLDER = join(SKILL_ROOT, 'nothing-design') // the folder to package
const DIST_DIR = join(SKILL_ROOT, 'dist')
const ARCHIVE_BASENAME = 'nothing-design-skill'
...
function main() {
  ...
  if (!existsSync(SKILL_FOLDER)) {
    ...
    console.error('        Run this script from the nothing-design-skill/ directory.')
    process.exit(1)
  }
  ...
  const archiveRootName = 'nothing-design'
  const ext = wantZip ? '.zip' : '.tar.gz'
  const outputFile = join(DIST_DIR, `${ARCHIVE_BASENAME}${ext}`)

  // 5. Build tar args
  //    We tar from SKILL_ROOT so the archive contains `nothing-design/` as the
...
function reportSuccess(outputFile) {
  ...
  console.log('')
  console.log('  Then restart Claude Code — the /nothing-design skill will be available.')
  console.log('')
}
