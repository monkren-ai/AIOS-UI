...
export const inputMessageVariants = cva(['flex flex-col gap-xs font-body'], {
  variants: {
    size: {
      sm: '',
      md: '',
...
/** textarea + 发送按钮的一行。 */
export const inputMessageControlVariants = cva(
  [
...
export const inputMessageFieldVariants = cva(
  [
...
/** 发送按钮：实心反相。 */
export const inputMessageSendVariants = cva(
  [
...
export const inputMessageSendLabelVariants = cva([])
...
/** 提示 + 计数一行，两端对齐。 */
export const inputMessageMetaVariants = cva(['flex items-center justify-between px-1'])

export const inputMessageHintVariants = cva([
  'font-mono text-micro uppercase tracking-widest text-foreground-disabled',
])

export const inputMessageCountVariants = cva([
  'font-mono text-micro uppercase tracking-widest text-foreground-disabled',
])
...
export type InputMessageSize = 'sm' | 'md' | 'lg' | keyof typeof LEGACY_SIZES

export function resolveInputMessageSize(size: InputMessageSize | null | undefined) {
