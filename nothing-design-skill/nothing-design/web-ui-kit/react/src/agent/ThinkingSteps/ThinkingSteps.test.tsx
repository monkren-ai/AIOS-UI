...
describe('ThinkingSteps', () => {
  ...
  it('renders status classes and data attributes', () => {
    ...
    expect(items[0]).toHaveClass('nothing-thinking-steps__item--done')
    expect(items[0]).toHaveAttribute('data-status', 'done')
    expect(items[1]).toHaveClass('nothing-thinking-steps__item--thinking')
    expect(items[1]).toHaveAttribute('data-status', 'thinking')
    expect(items[2]).toHaveClass('nothing-thinking-steps__item--pending')
  })
  ...
  it('supports compact variant', () => {
    ...
    expect(screen.getByText('THINKING').closest('[data-slot]')).toHaveClass(
      'nothing-thinking-steps--compact',
    )
  })
  ...
  it('supports custom className', () => {
    ...
    expect(root).toHaveClass('nothing-thinking-steps')
  })
