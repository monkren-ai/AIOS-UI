...
export const thinkingStepsVariants = cva('nothing-thinking-steps', {
  variants: {
    compact: {
  ...
  (
    ...
    return (
      <div
        ...
        {...props}
      >
        <div className="nothing-thinking-steps__header">
          <span className="nothing-thinking-steps__title">{title}</span>
          <span className="nothing-thinking-steps__count">
            {String(activeCount).padStart(2, '0')}/{String(steps.length).padStart(2, '0')}
          </span>
        </div>

        <ol className="nothing-thinking-steps__list" aria-label={`${title} steps`}>
          {steps.map((step, index) => {
            ...
            return (
              <li
                key={step.id}
                className={cn(
                  'nothing-thinking-steps__item',
                  `nothing-thinking-steps__item--${status}`,
                  isLast && 'nothing-thinking-steps__item--last',
                )}
                data-status={dataAttr(status)}
                style={{ '--step-index': index } as React.CSSProperties}
              >
                <div className="nothing-thinking-steps__marker" aria-hidden="true">
                  {indicatorState ? (
                    <ThinkingIndicator state={indicatorState} size="sm" />
                  ) : (
                    <span className="nothing-thinking-steps__dot" />
                  )}
                  {!isLast && <span className="nothing-thinking-steps__line" />}
                </div>
                <div className="nothing-thinking-steps__content">
                  <div className="nothing-thinking-steps__row">
                    <span className="nothing-thinking-steps__label">{step.title}</span>
                    <span className="nothing-thinking-steps__status">{statusLabels[status]}</span>
                  </div>
                  {step.content && (
                    <span className="nothing-thinking-steps__description">{step.content}</span>
                  )}
                </div>
              </li>
            )
