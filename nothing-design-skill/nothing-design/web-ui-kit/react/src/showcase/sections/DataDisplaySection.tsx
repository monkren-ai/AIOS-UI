    ...
    <CategorySection id="data-display" title={t('数据展示', 'Data Display')}>
      <DemoCard title={t('卡片', 'Cards')} variant="grid">
        <Card variant="default" title={t('默认卡片', 'Default Card')} action={t('更多', 'More')}>
          <p style={{ fontSize: 'var(--body)', margin: 0 }}>
            {t('带标题和操作的标准卡片。', 'Standard card with header and action.')}
          </p>
        </Card>
        <Card variant="raised" title={t('突出卡片', 'Raised Card')}>
          <p style={{ fontSize: 'var(--body)', margin: 0 }}>
            {t('具有明显背景的浮动界面。', 'Elevated surface with background distinction.')}
          </p>
        </Card>
        <Card variant="compact" title={t('紧凑', 'Compact')}>
          <p style={{ fontSize: 'var(--body)', margin: 0 }}>
            {t('为密集布局减少内边距。', 'Reduced padding for dense layouts.')}
          </p>
        </Card>
        <Card variant="technical" title="[ Technical ]">
          <p style={{ fontSize: 'var(--body)', margin: 0 }}>
            {t('等宽技术风格变体。', 'Monospace technical variant.')}
          </p>
        </Card>
        <Card
          variant="borderless"
        ...
        >
          <p style={{ fontSize: 'var(--body)', margin: 0 }}>
            {t('支持徽标、Logo 和媒体插槽。', 'Supports feature badge, logo and media slots.')}
          </p>
        </Card>
        <Card
          variant="default"
        ...
        >
          <p style={{ fontSize: 'var(--body)', margin: 0 }}>
            {t('在主体上方展示媒体内容。', 'Show media content above the body.')}
          </p>
        </Card>
      </DemoCard>
      ...
      <DemoCard title={t('手风琴', 'Accordion')}>
        <Accordion
          items={[
            {
              id: '1',
              title: t('设计理念', 'Design Philosophy'),
              content: t(
                '单色、零阴影、零渐变，以工业控制台的诚实感呈现信息。',
                'Monochrome, zero shadows, zero gradients, presenting information with the honesty of an industrial console.',
              ),
            },
            {
              id: '2',
              title: t('动效原则', 'Motion Principles'),
              content: t(
                '所有动效都服务于信息反馈，使用 spring 物理曲线保持一致的节奏。',
                'All motion serves informational feedback, using spring physics curves for a consistent rhythm.',
              ),
            },
            {
              id: '3',
              title: t('无障碍支持', 'Accessibility'),
              content: t(
                '完整的键盘导航、ARIA 状态与 reduced-motion 媒体查询支持。',
                'Full keyboard navigation, ARIA states and reduced-motion media query support.',
              ),
            },
          ]}
      ...
      <DemoCard title={t('数据网格', 'Data Grid')}>
        <DataTable
          ...
          rows={[
            {
              cells: { name: t('传感器 A', 'Sensor A'), status: t('在线', 'Online'), value: 42 },
              interactive: true,
              cellStatuses: [{ columnKey: 'status', status: 'good' }],
            },
            {
              cells: { name: t('传感器 B', 'Sensor B'), status: t('警告', 'Warning'), value: 78 },
              interactive: true,
              cellStatuses: [{ columnKey: 'status', status: 'warning' }],
            },
            {
              cells: { name: t('传感器 C', 'Sensor C'), status: t('离线', 'Offline'), value: 0 },
              interactive: true,
              cellStatuses: [{ columnKey: 'status', status: 'error' }],
            },
          ]}
        />
      </DemoCard>
