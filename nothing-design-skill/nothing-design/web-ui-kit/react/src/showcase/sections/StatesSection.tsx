  ...
  return (
    <CategorySection id="states" title={t('状态', 'States')}>
      <DemoCard title={t('状态', 'States')} variant="grid" last>
        ...
        <ErrorState
          headline={t('连接丢失', 'Connection Lost')}
          message={t('无法连接到服务器。', 'Unable to reach the server.')}
          onRetry={() => {}}
        />
        <EmptyState
          headline={t('暂无设备', 'No Devices')}
          description={t('配对设备以开始。', 'Pair a device to get started.')}
        />
        <DisabledState
          headline={t('功能锁定', 'Feature Locked')}
          description={t('需要高级套餐。', 'Requires premium plan.')}
        />
      </DemoCard>
      ...
      <DemoCard title={t('提示', 'Alert')}>
        <Alert
          title={t('注意！', 'Heads up!')}
          variant="default"
          style={{ maxWidth: '500px', marginBottom: 'var(--space-md)' }}
        >
          {t(
            '你可以通过 CLI 将组件添加到应用中。',
        ...
        {closableVisible && (
          ...
          >
            {t(
              '点击右侧关闭按钮，Alert 将以退出动效消失。',
              'Click the close button and the Alert will disappear with an exit motion.',
            )}
          </Alert>
        )}
        {!closableVisible && (
