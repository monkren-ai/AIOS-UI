...
export const spinnerVariants = cva(
  [
    'flex flex-col items-center rounded-lg border transition-colors duration-300 ease-nothing',
...
/** 转盘容器。尺寸决定盘面直径。 */
export const spinnerWheelVariants = cva(['relative mb-6'], {
  variants: {
    size: {
      sm: 'size-48',
      md: 'size-70',
...
export const spinnerPointerVariants = cva([
...
export const spinnerSvgVariants = cva([
  'size-full',
...
/** 扇区。 */
export const spinnerSectorVariants = cva(
  [
    'stroke-border [stroke-width:1]',
    'transition-[fill] duration-300 ease-nothing motion-reduce:transition-none',
  ],
  {
    variants: {
      isEven: {
...
export const spinnerTextVariants = cva(
  [
    'pointer-events-none font-mono text-label font-bold tracking-wide',
...
export type SpinnerVariant = 'soft' | 'outline' | 'destructive' | keyof typeof LEGACY_VARIANTS

export type SpinnerSize = 'sm' | 'md' | 'lg'

export function resolveSpinnerVariant(variant: SpinnerVariant | null | undefined) {
