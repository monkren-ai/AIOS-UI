    ...
    compoundVariants: [
      {
        variant: 'bracket',
        active: true,
        // 桌面：用方括号包住标签。窄屏：换回顶部的小圆点，
        // 靠 inset-x-0 + mx-auto 居中，不用 translate，RTL 下自动成立。
        class: [
...
/** 包住导航项与分隔符的行内容器。 */
export const navItemWrapperVariants = cva(['inline-flex items-center'])

/** pipe 变体的竖线分隔符。窄屏藏起来。 */
export const navSeparatorVariants = cva(['mx-1 text-border-visible max-[768px]:hidden'])
...
/** 返回按钮。窄屏钉在视口行首上角。 */
export const navBackVariants = cva([
...
export type NavigationVariant = 'default' | 'bracket' | 'pipe'
