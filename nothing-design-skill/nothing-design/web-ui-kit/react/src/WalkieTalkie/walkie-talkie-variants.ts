 ...
 * 不参与 §3 的强调层级词表。
 */
export const walkieTalkieVariants = cva(
  [
    'flex w-full select-none flex-col items-center rounded-lg border border-border bg-surface p-8',
...
/** 频道选择行。 */
export const walkieChannelVariants = cva(['mb-6 flex w-full items-center justify-center gap-2'])

export const walkieChannelLabelVariants = cva([
  'font-mono text-label uppercase tracking-[0.1em] text-foreground-muted',
  'transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none',
])

export const walkieChannelNumberVariants = cva([
  'font-mono text-base font-bold tabular-nums text-foreground-display',
  'transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none',
])

/** 频道加减按钮。 */
export const walkieChannelButtonVariants = cva([
...
export const walkiePttAreaVariants = cva(['relative mb-6 flex items-center justify-center'])

/** Push-to-talk 按钮。 */
export const walkiePttVariants = cva(
  [
 ...
 * 不会跟居中用的位移打架。
 */
export const walkiePulseVariants = cva(
  [
    'pointer-events-none absolute inset-0 z-1 m-auto size-20 rounded-full border-2 border-accent opacity-0',
  ],
  {
    variants: {
      transmitting: {
    ...
    defaultVariants: { transmitting: false, index: 0 },
  },
)

/** 状态读数。 */
export const walkieStatusVariants = cva(
  [
    'mb-6 font-mono text-sm uppercase tracking-[0.1em]',
    'transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none',
  ],
  {
    variants: {
...
/** 音量条容器。 */
export const walkieVolumeVariants = cva(['mb-2 flex h-8 items-end gap-0.5'])

/** 单根音量条。 */
export const walkieVolumeSegmentVariants = cva(
  ['w-2 transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none'],
  {
    variants: {
      filled: {
...
export const walkieVolumeLabelVariants = cva([
  'font-mono text-label uppercase tracking-widest text-foreground-disabled',
  'transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none',
])
