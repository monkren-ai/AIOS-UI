...
export const DoubleDown = React.forwardRef<HTMLDivElement, WidgetSubProps>(
  ({ theme, size, className, 'aria-label': ariaLabel, style, ...props }, ref) => {
    return (
      <div ref={ref} style={style} className={cn(widgetSubVariants({ theme, size }), `widget-card-wrapper ${className || ''}`.trim())} data-theme={dataAttr(theme)} data-size={dataAttr(size)} {...props} data-name="Double Down" aria-label={ariaLabel || "Double Down"}>
      ...
      <div className="absolute left-[8px] top-[8px] widget-card__svg--24 widget-card__icon" data-name="Spotify - Negative">
