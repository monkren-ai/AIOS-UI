...
export const Card = React.forwardRef<HTMLDivElement, CardProps>(
  ({ theme, size, className, 'aria-label': ariaLabel, style, variant = 'square', image = 'a', ...props }, ref) => {
    const imageSrc = typeof image === 'string' && image in IMAGE_PRESETS
    ...
    if (variant === 'pill') {
      return (
        <div
          ref={ref}
          style={style}
          className={cn(widgetSubVariants({ theme, size }), `grid-cols-[max-content] grid-rows-[max-content] inline-grid widget-leading-0 place-items-start widget-relative widget-shrink-0 widget-overflow-hidden ${className || ''}`.trim())}
        ...
        >
          ...
          <div className="widget-col-1 h-[136px] ml-[8.1px] mt-[8px] widget-relative widget-rounded-pill widget-row-1 w-[303.798px]" data-name="Image">
            <div className="absolute inset-0 overflow-hidden pointer-events-none widget-rounded-pill">
              <img alt="" className="absolute h-[148.71%] left-0 max-w-none top-[-7.82%] w-full" src={imageSrc} />
            </div>
          </div>
        </div>
      )
    ...
    return (
      <div
        ref={ref}
        style={style}
        className={cn(widgetSubVariants({ theme, size }), `widget-card-wrapper ${className || ''}`.trim())}
        data-theme={dataAttr(theme)}
        data-size={dataAttr(size)}
        data-variant={dataAttr(variant)}
        {...props}
      ...
      >
        ...
        <div className="absolute left-[8px] top-[8px] widget-rounded-16 size-[136px]" data-name="Image">
          <img alt="" className="absolute inset-0 max-w-none object-cover pointer-events-none widget-rounded-16 size-full" src={imageSrc} />
        </div>
      </div>
    )
  }
