...
export const toolbarVariants = cva(['inline-flex items-center gap-1'], {
  variants: {
    orientation: {
      horizontal: 'flex-row',
  ...
  ],
  {
    variants: {
      ...
      pressed: {
        ...
        false:
          'border-border bg-surface text-foreground hover:not-disabled:border-border-visible hover:not-disabled:bg-surface-raised hover:not-disabled:text-foreground-display',
      },
    },
...
/** 工具条分组容器。 */
export const toolbarGroupVariants = cva(['inline-flex items-center gap-1'], {
  variants: {
    orientation: {
      horizontal: 'flex-row',
      vertical: 'flex-col items-start',
