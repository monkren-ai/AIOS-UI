...
function Frame45() {
  return (
    <div className="content-stretch flex flex-col gap-[4px] items-start widget-relative widget-shrink-0" aria-hidden="true">
      <Frame6 />
      <Frame9 />
    </div>
  );
...
export const Widget = React.forwardRef<HTMLDivElement, WidgetSubProps>(
  ({ theme, size, className, 'aria-label': ariaLabel, style, ...props }, ref) => {
    return (
      <div ref={ref} style={style} className={cn(widgetSubVariants({ theme, size }), `widget-bg-dark content-stretch flex flex-col h-[152px] items-start justify-between p-[16px] widget-relative widget-card--rounded widget-shrink-0 w-[226px] ${className || ''}`.trim())} data-theme={dataAttr(theme)} data-size={dataAttr(size)} {...props} data-name="Widget" aria-label={ariaLabel || "Widget"}>
