...
export const Recording = React.forwardRef<HTMLDivElement, RecordingProps>(
  ({ theme, size, className, 'aria-label': ariaLabel, style, variant = 'pill', time = '00:00:05', ...props }, ref) => {
    if (variant === 'rec') {
      return (
        <div ref={ref} style={style} className={cn(widgetSubVariants({ theme, size }), `widget-grid-auto ${className || ''}`.trim())} data-theme={dataAttr(theme)} data-size={dataAttr(size)} data-variant={dataAttr(variant)} {...props} data-name="Record" aria-label={ariaLabel || "Record"}>
    ...
    return (
      <div ref={ref} style={style} className={cn(widgetSubVariants({ theme, size }), `widget-card widget-card--152 widget-card--pill widget-card--light content-stretch flex gap-[6px] items-center justify-center widget-relative widget-shrink-0 ${className || ''}`.trim())} data-theme={dataAttr(theme)} data-size={dataAttr(size)} data-variant={dataAttr(variant)} {...props} data-name="Recording" aria-label={ariaLabel || "Recording"}>
      <div className="widget-card__icon widget-relative widget-shrink-0 size-[8px]" data-name="Dot">
        <svg className="nothing-widget-icon-svg" fill="none" preserveAspectRatio="xMidYMid meet" viewBox="0 0 8 8">
          <circle cx="4" cy="4" fill="var(--widget-primary)" fillOpacity="1" id="Dot" r="4" />
        </svg>
      </div>
      <p className=" widget-text widget-text--ndot widget-text--12 widget-text--grey2 widget-text--center widget-text--nowrap">{time}</p>
    </div>
    )
  }
)
