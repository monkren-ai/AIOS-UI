...
export const Active = React.forwardRef<HTMLDivElement, ActiveProps>(
  ({ theme, size, className, 'aria-label': ariaLabel, style, variant = 'active', label, ...props }, ref) => {
    const displayLabel = label ?? DEFAULT_LABELS[variant]
    return (
      <div ref={ref} style={style} className={cn(widgetSubVariants({ theme, size }), `widget-card widget-card--152 widget-card--pill widget-card--light content-stretch flex flex-col gap-[12px] items-center justify-center widget-relative widget-shrink-0 ${className || ''}`.trim())} data-theme={dataAttr(theme)} data-size={dataAttr(size)} data-variant={dataAttr(variant)} {...props} data-name="Active" aria-label={ariaLabel || "Active"}>
