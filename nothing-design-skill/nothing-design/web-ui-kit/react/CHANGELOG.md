...
- **产品名**：Nothing UI → **AIOS UI**
- **包名**：`nothing-ui-kit-react` / `nothing-ui` → **`aios-ui-kit`**（导入写作 `aios-ui-kit/button`）
- **仓库**：`monkren-ai/Nothing-UI` → **`monkren-ai/AIOS-UI`**（站点 `base` / `basename` 同步为 `/AIOS-UI/`）
