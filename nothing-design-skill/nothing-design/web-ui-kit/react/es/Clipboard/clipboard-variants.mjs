...
//#region src/Clipboard/clipboard-variants.ts
/**
* Clipboard 卡片容器。
*
...
/** 头部一行：标题在前，计数在后。 */
const clipboardHeaderVariants = cva(["mb-lg flex w-full items-baseline justify-between"]);
const clipboardTitleVariants = cva(["font-mono uppercase tracking-widest text-foreground-muted", "transition-colors duration-200 ease-nothing motion-reduce:transition-none"], {
	variants: { size: {
		sm: "text-micro",
		md: "text-label",
		lg: "text-caption"
...
const clipboardCountVariants = cva(["font-mono text-caption text-foreground-disabled", "transition-colors duration-200 ease-nothing motion-reduce:transition-none"]);
const clipboardListVariants = cva(["mb-lg flex flex-col gap-xs"]);
/** 单条剪贴记录。整行可点，所以高度走 36 / 44 / 52 的触达基线。 */
const clipboardItemVariants = cva([
...
const clipboardItemContentVariants = cva(["flex min-w-0 flex-1 flex-col gap-2xs"]);
...
const clipboardTimeVariants = cva(["text-start font-mono text-caption text-foreground-disabled", "transition-colors duration-200 ease-nothing motion-reduce:transition-none"]);
/** [COPIED] 标记。默认透明，靠 group-data 淡入。 */
const clipboardCopiedVariants = cva([
...
/** 删除按钮。 */
const clipboardDeleteVariants = cva([
...
/** 清空按钮。 */
const clipboardClearVariants = cva([
...
function resolveClipboardSize(size) {
	if (!size) return void 0;
	return LEGACY_SIZES[size] ?? size;
}
//#endregion
export { clipboardClearVariants, clipboardCopiedVariants, clipboardCountVariants, clipboardDeleteVariants, clipboardHeaderVariants, clipboardItemContentVariants, clipboardItemVariants, clipboardListVariants, clipboardTextVariants, clipboardTimeVariants, clipboardTitleVariants, clipboardVariants, resolveClipboardSize };

//# sourceMappingURL=clipboard-variants.mjs.map