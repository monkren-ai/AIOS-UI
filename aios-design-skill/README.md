...
Tell Claude `/aios-design` or say "AIOS style" and it generates UI following these principles:
...
Package the entire `aios-design/` skill folder into a `.tar.gz` (no npm install required — uses the system `tar` command, built into Windows 10+, macOS, and Linux):

```sh
node scripts/download-skill.mjs
# → dist/aios-design-skill.tar.gz
```

Then install:

```sh
tar -xzf dist/aios-design-skill.tar.gz -C ~/.claude/skills/
```
