...
export const tagVariants = cva(
  ...
  ],
  {
    variants: {
      variant: {
        secondary:
          'border-border-visible text-foreground-muted hover:not-data-disabled:border-foreground-muted hover:not-data-disabled:text-foreground hover:not-data-disabled:scale-[1.02] motion-reduce:hover:scale-100',
        ...
        outline:
          'border-border text-foreground-subtle hover:not-data-disabled:border-border-visible hover:not-data-disabled:text-foreground',
        ghost:
          'border-transparent text-foreground-muted hover:not-data-disabled:bg-muted hover:not-data-disabled:text-foreground-display',
...
export const tagsVariants = cva(['flex flex-wrap gap-1'], {
  variants: {
    proximity: {
      true: 'group/tags',
      false: '',
...
export type TagShape = 'pill' | 'technical'

export function resolveTagVariant(variant: TagVariant | null | undefined) {
  if (!variant) return undefined
  return (LEGACY_VARIANTS as Record<string, string>)[variant] ?? variant
}

/** 老调用点把形状写在 variant 上时，把它翻译成 shape。 */
export function resolveTagShape(
  variant: TagVariant | null | undefined,
  shape: TagShape | null | undefined,
): TagShape | undefined {
  if (shape) return shape
  if (!variant) return undefined
  return (LEGACY_SHAPES as Record<string, TagShape>)[variant]
}
