 ...
 */
export const pomodoroVariants = cva(
  [
...
/** 顶部标题。 */
export const pomodoroTitleVariants = cva([
  'font-mono text-label uppercase tracking-widest text-foreground-muted',
  'transition-colors duration-[350ms] ease-aios motion-reduce:transition-none',
])

/** 已完成的番茄数。 */
export const pomodoroCountVariants = cva([
  'font-mono text-caption tabular-nums text-foreground-disabled',
  'transition-colors duration-[350ms] ease-aios motion-reduce:transition-none',
])

/** 倒计时读数。休息段跟着状态标一起转绿。 */
export const pomodoroTimerVariants = cva(
  [
    'mb-4 font-display text-display-xl font-semibold leading-none tracking-[-0.02em] tabular-nums',
    'transition-colors duration-[350ms] ease-aios motion-reduce:transition-none',
  ],
  {
    variants: {
...
/** [WORK] / [BREAK] 状态标。 */
export const pomodoroStatusVariants = cva(
  [
    'mb-6 font-mono text-sm uppercase tracking-[0.12em]',
    'transition-colors duration-[350ms] ease-aios motion-reduce:transition-none',
  ],
  {
    variants: {
...
export const pomodoroSegmentVariants = cva(
  ['flex-1 transition-colors duration-[350ms] ease-aios motion-reduce:transition-none'],
  {
    ...
    compoundVariants: [
      { filled: true, phase: 'work', class: 'bg-accent' },
      { filled: true, phase: 'break', class: 'bg-success' },
    ],
    defaultVariants: { filled: false, phase: 'work' },
  },
)

/** Start / Reset 按钮。 */
export const pomodoroButtonVariants = cva(
  [
