...
export const alertIconVariants = cva(
  ['flex shrink-0 items-start leading-none text-foreground-muted'],
  {
    variants: {
      variant: {
        soft: '',
...
/** 标题槽位。 */
export const alertTitleVariants = cva(['font-mono font-bold tracking-tight'], {
  variants: {
    variant: {
...
/** 正文槽位。 */
export const alertMessageVariants = cva([''], {
  variants: {
    variant: {
...
export type AlertVariant = 'soft' | 'destructive' | keyof typeof LEGACY_VARIANTS

export type AlertSize = 'sm' | 'md' | 'lg'

export function resolveAlertVariant(variant: AlertVariant | null | undefined) {
