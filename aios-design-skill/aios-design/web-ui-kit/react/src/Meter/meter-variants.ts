 ...
 */
export const meterVariants = cva(['flex flex-col gap-2'], {
  variants: {
    size: { sm: '', md: '', lg: '' },
  },
...
/** 分段轨。高度跟着 size 走。 */
export const meterTrackVariants = cva(['relative flex w-full overflow-hidden gap-0.5'], {
  variants: {
    size: {
      sm: 'h-1.5',
      md: 'h-2.5',
...
export const meterSegmentVariants = cva(
  [
    'flex-1 rounded-none',
...
export const meterValueVariants = cva(['font-mono tabular-nums'], {
  variants: {
    size: {
      sm: 'text-xs',
      md: 'text-sm',
  ...
  defaultVariants: { size: 'md', zone: 'good' },
})

/** 临界值竖标：从轨顶到轨底的 1px 细线，标出 low / high 的位置。 */
export const meterMarkerVariants = cva(['absolute inset-y-0 w-px bg-border-visible'])

export type MeterSize = 'sm' | 'md' | 'lg'

export type MeterZone = 'good' | 'warning' | 'critical'
