...
describe('DotMatrix', () => {
  ...
  it('keeps the BEM hooks that styles/glyph.css selects on', () => {
    ...
    const root = container.querySelector('[data-slot="dot-matrix"]')
    expect(root).toHaveClass('aios-dot-matrix')
    expect(root).toHaveClass('aios-dot-matrix--glyph')
    const all = dots(container)
    expect(all[0]).toHaveClass('aios-dot-matrix__dot')
    expect(all[0]).toHaveClass('aios-dot-matrix__dot--active')
    expect(all[1]).toHaveClass('aios-dot-matrix__dot--dim')
  })
