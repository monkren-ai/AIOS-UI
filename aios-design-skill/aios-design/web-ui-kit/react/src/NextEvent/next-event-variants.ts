...
/** 月份缩写。 */
export const nextEventMonthVariants = cva([
  'shrink-0 font-display text-xs leading-none tracking-normal text-accent',
])

/** 倒计时。24 小时内的事件（high）转成红色并加粗。 */
export const nextEventCountdownVariants = cva(
  ['ms-auto shrink-0 font-mono text-micro uppercase tracking-widest tabular-nums'],
  {
    variants: {
      priority: {
