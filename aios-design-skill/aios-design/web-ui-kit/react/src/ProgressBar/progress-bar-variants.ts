 ...
 */
export const progressBarVariants = cva(['flex flex-col gap-2'], {
  variants: {
    variant: {
      segmented: '',
      slim: '',
...
export const progressTrackVariants = cva(['relative flex w-full overflow-hidden'], {
  variants: {
    variant: {
      segmented: 'gap-0.5',
      slim: 'gap-px',
  ...
  compoundVariants: [{ variant: 'slim', class: 'h-1' }],
  defaultVariants: { variant: 'segmented', size: 'md', indeterminate: false },
})
...
export const progressSegmentVariants = cva(
  ...
  ],
  {
    ...
    defaultVariants: { state: 'empty', size: 'md', variant: 'segmented' },
  },
)
...
/** 读数行的数值。 */
export const progressValueVariants = cva(['font-mono text-base tabular-nums'], {
  variants: {
    status: {
...
export type ProgressBarVariant = 'segmented' | 'slim' | keyof typeof LEGACY_VARIANTS

export type ProgressBarSize = 'sm' | 'md' | 'lg' | keyof typeof LEGACY_SIZES

export type ProgressStatus = 'default' | 'good' | 'warning' | 'overlimit' | 'error'

export function resolveProgressBarSize(size: ProgressBarSize | null | undefined) {
  if (!size) return undefined
  return (LEGACY_SIZES as Record<string, string>)[size] ?? size
}

export function resolveProgressBarVariant(variant: ProgressBarVariant | null | undefined) {
