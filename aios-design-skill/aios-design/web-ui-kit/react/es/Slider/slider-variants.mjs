...
//#region src/Slider/slider-variants.ts
/**
* Slider 的视觉变体。
*
...
*/
const sliderVariants = cva(["group/slider flex w-full select-none flex-col gap-2 [-webkit-tap-highlight-color:transparent]"], {
	variants: {
		size: {
			sm: "",
			md: "",
...
/** label + value 的一行。 */
const sliderHeaderVariants = cva(["flex items-center justify-between"]);
/** 字段标签。 */
const sliderLabelVariants = cva(["font-mono text-label uppercase tracking-wider text-foreground-muted"]);
/** 当前值。 */
const sliderValueVariants = cva(["font-mono text-label uppercase tracking-wider text-foreground"]);
/** 轨道容器（Base UI Slider.Control）。高度走 36 / 44 / 52 的触达基线。 */
const sliderControlVariants = cva(["relative flex w-full touch-none items-center"], {
	variants: { size: {
		sm: "h-9",
		md: "h-11",
		lg: "h-13"
...
function resolveSliderVariant(variant) {
	if (!variant) return void 0;
	return LEGACY_VARIANTS[variant] ?? variant;
}
//#endregion
export { resolveSliderVariant, sliderControlVariants, sliderFillVariants, sliderHeaderVariants, sliderLabelVariants, sliderThumbVariants, sliderTrackVariants, sliderValueVariants, sliderVariants };

//# sourceMappingURL=slider-variants.mjs.map