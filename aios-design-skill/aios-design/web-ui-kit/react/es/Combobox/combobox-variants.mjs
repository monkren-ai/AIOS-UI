...
*/
const comboboxVariants = cva(["relative flex w-full flex-col gap-1"], {
	variants: {
		size: {
			sm: "",
			md: "",
...
/** 字段标签。 */
const comboboxLabelVariants = cva(["font-mono uppercase tracking-wider text-foreground-muted"], {
	variants: {
		size: {
			sm: "text-micro",
			md: "text-label",
...
const comboboxItemVariants = cva([
	...
	variants: {
		...
		selected: {
			true: ["bg-muted text-foreground-display", "before:absolute before:inset-y-0 before:start-0 before:w-0.5 before:bg-accent before:content-['']"],
			false: ""
		},
		highlighted: {
...
/** 错误文案。 */
const comboboxErrorVariants = cva(["mt-xs font-mono text-label uppercase tracking-wide text-accent"]);
//#endregion
export { comboboxClearVariants, comboboxContentVariants, comboboxControlVariants, comboboxEmptyVariants, comboboxErrorVariants, comboboxIconVariants, comboboxInputVariants, comboboxItemIndicatorVariants, comboboxItemVariants, comboboxLabelVariants, comboboxListVariants, comboboxPositionerVariants, comboboxVariants };

//# sourceMappingURL=combobox-variants.mjs.map