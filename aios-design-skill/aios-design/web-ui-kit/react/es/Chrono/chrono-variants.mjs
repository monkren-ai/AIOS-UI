...
//#region src/Chrono/chrono-variants.ts
/**
* Chrono（秒表）的视觉变体。
*
...
const chronoButtonVariants = cva([
	...
	variants: { action: {
		start: "flex-1 border-none bg-foreground-display text-black hover:not-disabled:opacity-85 active:not-disabled:opacity-70",
		pause: "flex-1 border-none bg-accent text-foreground-display hover:not-disabled:opacity-85 active:not-disabled:opacity-70",
		...
		reset: "w-full border border-border-visible bg-transparent text-foreground-muted hover:not-disabled:border-foreground-muted hover:not-disabled:text-foreground-display active:not-disabled:border-foreground"
	} },
	defaultVariants: { action: "start" }
});
/** 圈速列表容器。滚动条收成 4px 的细条。 */
const chronoLapsVariants = cva([
...
//# sourceMappingURL=chrono-variants.mjs.map