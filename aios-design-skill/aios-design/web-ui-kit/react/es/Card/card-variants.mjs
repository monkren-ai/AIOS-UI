...
/** v1 里塞在 variant 上的密度。 */
const LEGACY_CARD_SIZES = { compact: "sm" };
/** v1 里塞在 variant 上的形状。 */
const LEGACY_CARD_SHAPES = { technical: "technical" };
function resolveCardVariant(variant) {
	if (!variant) return void 0;
	return LEGACY_CARD_VARIANTS[variant] ?? variant;
}
/** 老调用点把密度写在 variant 上（`compact`）时，把它翻译成 size。 */
function resolveCardSize(variant, size) {
	if (size) return size;
	if (!variant) return void 0;
	return LEGACY_CARD_SIZES[variant];
}
/** 老调用点把形状写在 variant 上（`technical`）时，把它翻译成 shape。 */
function resolveCardShape(variant, shape) {
	if (shape) return shape;
	if (!variant) return void 0;
	return LEGACY_CARD_SHAPES[variant];
}
/**
* Widget 卡片的视觉变体。
*
...
/** Widget 标题。 */
const widgetCardTitleVariants = cva(["text-micro uppercase leading-snug tracking-wider"], {
	variants: { theme: {
...
/** Widget 副标题。 */
const widgetCardSubtitleVariants = cva(["text-micro uppercase leading-snug tracking-wider"], {
	variants: { theme: {
...
function resolveWidgetCardSize(size) {
	if (!size) return void 0;
	return LEGACY_WIDGET_SIZES[size] ?? size;
}
function resolveWidgetCardDensity(variant) {
	if (!variant) return void 0;
	return LEGACY_WIDGET_DENSITIES[variant] ?? variant;
}
//#endregion
export { contentCardVariants, resolveCardShape, resolveCardSize, resolveCardVariant, resolveWidgetCardDensity, resolveWidgetCardSize, widgetCardSubtitleVariants, widgetCardTitleVariants, widgetCardValueVariants, widgetCardVariants };
