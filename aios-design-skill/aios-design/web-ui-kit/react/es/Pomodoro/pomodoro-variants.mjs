...
/** 顶部标题。 */
const pomodoroTitleVariants = cva(["font-mono text-label uppercase tracking-widest text-foreground-muted", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"]);
/** 已完成的番茄数。 */
const pomodoroCountVariants = cva(["font-mono text-caption tabular-nums text-foreground-disabled", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"]);
/** 倒计时读数。休息段跟着状态标一起转绿。 */
const pomodoroTimerVariants = cva(["mb-4 font-display text-display-xl font-semibold leading-none tracking-[-0.02em] tabular-nums", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"], {
	variants: { phase: {
...
/** [WORK] / [BREAK] 状态标。 */
const pomodoroStatusVariants = cva(["mb-6 font-mono text-sm uppercase tracking-[0.12em]", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"], {
	variants: { phase: {
		work: "text-accent",
		break: "text-success"
	} },
...
const pomodoroSegmentVariants = cva(["flex-1 transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"], {
	variants: {
		filled: {
			true: "",
			false: "bg-border"
