...
*/
const photoCarouselVariants = cva(["flex w-full flex-col rounded-lg border border-border bg-surface p-8", "transition-colors duration-[350ms] ease-aios motion-reduce:transition-none"], {
	variants: {
		orientation: {
			horizontal: "",
			vertical: ""
...
/** 舞台。16:9，裁掉溢出。 */
const carouselContainerVariants = cva(["relative mb-6 aspect-video w-full overflow-hidden border border-border bg-surface-raised", "transition-colors duration-[350ms] ease-aios motion-reduce:transition-none"]);
/** 单张幻灯片。淡入淡出，非当前张保持在 DOM 里但透明。 */
const carouselSlideVariants = cva(["absolute inset-0 flex size-full flex-col items-center justify-center text-foreground", "transition-opacity duration-[350ms] ease-aios motion-reduce:transition-none"], {
	variants: { active: {
		true: "opacity-100",
		false: "opacity-0"
	} },
...
/** 没有图片时的几何占位图标。 */
const carouselSlideIconVariants = cva(["mb-2 stroke-foreground-muted", "transition-[stroke] duration-[350ms] ease-aios motion-reduce:transition-none"]);
/** 幻灯片上的文字块。 */
const carouselSlideTextVariants = cva(["mt-2 flex flex-col gap-0.5 text-center"]);
const carouselSlideTitleVariants = cva(["font-body text-subheading text-foreground", "transition-colors duration-[350ms] ease-aios motion-reduce:transition-none"]);
...
const carouselCounterVariants = cva(["font-mono text-caption tabular-nums text-foreground-muted", "transition-colors duration-[350ms] ease-aios motion-reduce:transition-none"]);
//#endregion
