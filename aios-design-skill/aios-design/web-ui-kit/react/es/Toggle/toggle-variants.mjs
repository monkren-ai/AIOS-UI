...
//#region src/Toggle/toggle-variants.ts
/**
* Toggle 的视觉变体。
*
...
const toggleVariants = cva([
	...
	variants: {
		variant: {
			...
			outline: "rounded-sm border border-border-visible bg-transparent text-foreground hover:not-disabled:border-foreground-muted pressed:border-accent pressed:bg-accent-subtle pressed:font-bold pressed:text-accent",
			ghost: "rounded-none border border-transparent bg-transparent text-foreground-muted hover:not-disabled:bg-muted pressed:bg-muted pressed:font-bold pressed:text-foreground-display"
		},
		size: {
...
* 逻辑属性保证 RTL 下自动镜像。
*/
const toggleGroupVariants = cva(["inline-flex flex-wrap items-center"], {
	variants: { variant: {
		soft: "gap-1",
		outline: [
			"gap-0",
...
function resolveToggleVariant(variant) {
	if (!variant) return void 0;
	return LEGACY_VARIANTS[variant] ?? variant;
}
//#endregion
export { resolveToggleVariant, toggleGroupVariants, toggleVariants };

//# sourceMappingURL=toggle-variants.mjs.map