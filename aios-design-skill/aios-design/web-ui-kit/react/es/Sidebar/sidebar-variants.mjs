...
//#region src/Sidebar/sidebar-variants.ts
/**
* Sidebar 的视觉变体。
*
...
/** 顶部自定义区。 */
const sidebarHeaderVariants = cva(["shrink-0 border-b border-border p-4"]);
/** 折叠开关。 */
...
/** 导航列表。 */
const sidebarListVariants = cva(["m-0 flex-1 list-none overflow-y-auto py-1"]);
/** 列表项容器。 */
const sidebarItemVariants = cva(["m-0"], {
	variants: { active: {
		true: "",
		false: ""
	} },
...
/** 列表项图标。 */
const sidebarItemIconVariants = cva(["inline-flex size-5 shrink-0 items-center justify-center"]);
/** 列表项文字。折叠时不渲染。 */
const sidebarItemLabelVariants = cva(["flex-1 overflow-hidden text-ellipsis font-body text-sm"]);
/** 列表项角标。 */
const sidebarItemBadgeVariants = cva(["min-w-5 shrink-0 rounded-pill bg-accent px-1 py-0.5 text-center", "font-mono text-caption text-foreground-display"]);
/** 底部自定义区。 */
const sidebarFooterVariants = cva(["shrink-0 border-t border-border p-4"]);
//#endregion
export { sidebarFooterVariants, sidebarHeaderVariants, sidebarItemBadgeVariants, sidebarItemIconVariants, sidebarItemLabelVariants, sidebarItemLinkVariants, sidebarItemVariants, sidebarListVariants, sidebarToggleVariants, sidebarVariants };

//# sourceMappingURL=sidebar-variants.mjs.map