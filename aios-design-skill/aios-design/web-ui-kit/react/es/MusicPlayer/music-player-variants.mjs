...
const musicPlayerVariants = cva([], {
	variants: {
		variant: {
			default: ["flex w-full flex-col rounded-lg border border-border bg-surface p-8", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"].join(" "),
			compact: ["flex w-full flex-col gap-3 rounded-lg border-0 bg-[var(--widget-bg)] p-4", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"].join(" "),
			mini: ""
		},
		recording: {
			true: "",
...
const playerTitleVariants = cva(["mb-1 font-body text-heading font-bold tracking-[-0.02em] text-foreground-display", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"]);
const playerArtistVariants = cva(["font-mono text-sm uppercase tracking-widest text-foreground-muted", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"]);
...
/** 进度条的单个刻度。 */
const playerProgressSegmentVariants = cva(["flex-1 transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"], {
	variants: { filled: {
		true: "bg-accent",
		false: "bg-border"
	} },
...
/** 当前时间 / 总时长。 */
const playerTimeVariants = cva(["flex justify-between font-mono text-caption tabular-nums text-foreground-disabled", "transition-colors duration-[350ms] ease-nothing motion-reduce:transition-none"]);
...
const playerCompactProgressFillVariants = cva(["h-full rounded-[1px] bg-[var(--widget-white)]", "transition-[width] duration-[350ms] ease-nothing motion-reduce:transition-none"]);
