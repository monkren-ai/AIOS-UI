...
| Agent | `PromptBox`、`KeywordTag`、`ContextBar`、`ToolCallRow`、`Reasoning`、`Plan`、`Sources`、`Confirmation`、`WebSearch`、`CodeDiff`、`Terminal`、`SubagentList`、`AgentOrb`、`ActivityLabel`、`AssistantPanel`、`ApprovalGate`、`PlanCard`、`ProgressTrace`、`ThinkingIndicator`、`ThinkingSteps` | `aios-ui-kit/agent`、`aios-ui-kit/conversation`、`aios-ui-kit/code-diff` |
...
Agent 分类共 23 个独立文档入口；AIOS 原生的 `AgentOrb`、`ActivityLabel`、`AssistantPanel`、`ApprovalGate`、`PlanCard`、`ProgressTrace`、`ThinkingIndicator` 与 `ThinkingSteps` 不再只藏在聚合页。
