...
| 维度 | v1（旧） | v2（新） |
|---|---|---|
| 样式载体 | 每组件一个 `.css` + `aios-*` BEM 类 | Tailwind v4 工具类，写在 CVA 里 |
...
| `text-accent` / `bg-accent` / `bg-accent-subtle` | `--accent`（#D71921，AIOS 红） |
...
`ease-aios`（默认缓动）· `ease-back` · `ease-spring-fast|moderate|slow` · `animate-agent-breathe|pulse|step`
