...
<script>
  // 数字时钟
  const digitalClock = new AIOSClock(document.getElementById('digitalClock'), {
    type: 'digital',
    theme: 'dark',
    updateInterval: 1000
  });
  ...
  // 仪表盘时钟
  const gaugeClock = new AIOSClock(document.getElementById('gaugeClock'), {
    type: 'gauge',
    theme: 'dark',
    updateInterval: 1000
  });
  ...
  // 圆形双圈时钟 — 上方小时/下方分钟（红色）
  const dualRingClock = new AIOSClock(document.getElementById('dualRingClock'), {
    type: 'dual-ring',
    theme: 'dark',
    updateInterval: 1000
  });
  ...
  // 叠加字体时钟 — 红色底层+深色/白色顶层
  const overlayClock = new AIOSClock(document.getElementById('overlayClock'), {
    type: 'overlay',
    theme: 'dark',
    updateInterval: 1000
  });
...
<script>
  // 标准条形电池
  const battery = new AIOSBattery(document.getElementById('batteryWidget'), {
    level: 75,
    isCharging: false,
    theme: 'dark'
  });
  ...
  // 环形进度电池 — 圆形双圈+外圈进度条
  const ringBattery = new AIOSBattery(document.getElementById('ringBattery'), {
    level: 75,
    isCharging: false,
    variant: 'ring',
    theme: 'dark'
...
<script>
  // 紧凑日历
  const compact = new AIOSCalendar(document.getElementById('compactCalendar'), {
    type: 'compact'
  });

  // 完整日历
  const full = new AIOSCalendar(document.getElementById('fullCalendar'), {
    type: 'full'
  });
</script>
```
...
<script>
  const monitor = new AIOSSystemMonitor(document.getElementById('systemMonitor'), {
    cpuUsage: 38,
    ramUsage: 65,
    storageUsage: 42,
    networkSpeed: 156,
...
<script>
  const player = new AIOSMusicPlayer(document.getElementById('musicPlayer'), {
    title: 'Song Title',
    artist: 'Artist Name',
    album: 'Album Name',
    isPlaying: false,
...
<script>
  const carousel = new AIOSPhotoCarousel(document.getElementById('photoCarousel'), {
    images: [
...
<script>
  const primaryBtn = new AIOSButton(document.getElementById('primaryBtn'), {
    variant: 'primary',
    label: 'Confirm',
    disabled: false
  });

  const destructiveBtn = new AIOSButton(document.getElementById('destructiveBtn'), {
    variant: 'destructive',
    label: 'Delete',
    disabled: false
  });
...
<script>
  const underlineInput = new AIOSInput(document.getElementById('underlineInput'), {
    style: 'underline',
    label: 'USERNAME',
  ...
  const borderedInput = new AIOSInput(document.getElementById('borderedInput'), {
    style: 'bordered',
    label: 'EMAIL',
...
<script>
  const toggle = new AIOSToggle(document.getElementById('toggle'), {
    checked: false,
    disabled: false
  });
...
<script>
  const pillTag = new AIOSTag(document.getElementById('pillTag'), {
    variant: 'pill',
    label: 'Active',
    active: true,
    removable: false
  ...
  const techTag = new AIOSTag(document.getElementById('techTag'), {
    variant: 'technical',
    label: 'V2.1',
    active: false,
    removable: true
...
<script>
  const segmented = new AIOSSegmentedControl(document.getElementById('segmented'), {
...
<script>
  const desktopNav = new AIOSNavigation(document.getElementById('desktopNav'), {
    layout: 'desktop',
    style: 'bracket',
    items: [
  ...
  const mobileNav = new AIOSNavigation(document.getElementById('mobileNav'), {
    layout: 'mobile',
    style: 'pipe',
    items: [
      { label: 'HOME', href: '#home' },
...
<script>
  const defaultCard = new AIOSCard(document.getElementById('defaultCard'), {
    variant: 'default',
    title: 'SYSTEM STATUS',
    subtitle: 'All services operational'
  });

  const techCard = new AIOSCard(document.getElementById('techCard'), {
    variant: 'technical',
    title: 'LATENCY',
    subtitle: '12ms avg'
  });
...
<script>
  const dataRow = new AIOSDataRow(document.getElementById('dataRow'), {
    label: 'CPU USAGE',
    value: '67%',
    status: 'warning'
  });
...
<script>
  const dataGrid = new AIOSDataGrid(document.getElementById('dataGrid'), {
...
<script>
  const heroProgress = new AIOSProgressBar(document.getElementById('heroProgress'), {
    size: 'hero',
    segments: 10,
    filled: 7,
    label: 'COMPLETION'
  ...
  const standardProgress = new AIOSProgressBar(document.getElementById('standardProgress'), {
    size: 'standard',
    segments: 5,
    filled: 3,
    label: 'STEPS'
...
<script>
  const modal = new AIOSModal(document.getElementById('modal'), {
...
<script>
  const dropdown = new AIOSDropdown(document.getElementById('dropdown'), {
    label: 'SELECT OPTION',
    options: [
...
<script>
  const sheet = new AIOSBottomSheet(document.getElementById('bottomSheet'), {
    title: 'OPTIONS',
    visible: false
  });
...
<script>
  const dateNav = new AIOSDateNav(document.getElementById('dateNav'), {
    label: 'MAY 2026',
...
<script>
  const loadingState = new AIOSState(document.getElementById('loadingState'), {
    type: 'loading',
    message: 'Loading data...'
  });

  const errorState = new AIOSState(document.getElementById('errorState'), {
    type: 'error',
...
<script>
  const caffeinate = new AIOSCaffeinate(document.getElementById('caffeinate'), {
    level: 120,
    maxLevel: 400,
    halfLifeHours: 5,
    drinks: [
...
<script>
  const clipboard = new AIOSClipboard(document.getElementById('clipboard'), {
    entries: [
...
<script>
  const pomodoro = new AIOSPomodoro(document.getElementById('pomodoro'), {
    workMinutes: 25,
    breakMinutes: 5,
    totalRounds: 4,
    currentRound: 1,
...
<script>
  const walkieTalkie = new AIOSWalkieTalkie(document.getElementById('walkieTalkie'), {
    channel: 'CHANNEL 1',
...
<script>
  const sunDial = new AIOSSunDial(document.getElementById('sunDial'), {
    sunrise: '06:12',
    sunset: '19:48',
    currentTime: '12:30',
    latitude: 39.9,
...
<script>
  const ageMotion = new AIOSAgeMotion(document.getElementById('ageMotion'), {
...
<script>
  const chrono = new AIOSChrono(document.getElementById('chrono'), {
    elapsedMs: 0,
    isRunning: false,
    laps: []
  });
...
<script>
  const spinner = new AIOSSpinner(document.getElementById('spinner'), {
...
<script>
  const worldClock = new AIOSWorldClock(document.getElementById('worldClock'), {
    timezones: [
...
<script>
  const dateWidget = new AIOSDate(document.getElementById('dateWidget'), {
    type: 'rect',
    theme: 'dark'
  });
</script>
...
<script>
  const nextEventWidget = new AIOSNextEvent(document.getElementById('nextEventWidget'), {
    theme: 'dark',
...
<script>
  const quotesWidget = new AIOSQuotes(document.getElementById('quotesWidget'), {
    theme: 'dark',
...
<script>
  const taskbarWidget = new AIOSTaskbar(document.getElementById('taskbarWidget'), {
    theme: 'dark',
...
<script>
  const dotMatrix = new AIOSDotMatrix(document.getElementById('dotMatrix'), {
    rows: 8,
    cols: 8,
    dotSize: 'sm',
    theme: 'dark'
...
<script>
  const quickToggle = new AIOSQuickToggle(document.getElementById('quickToggle'), {
    variant: 'circle',
    theme: 'light',
    label: 'Active'
  });
...
<script>
  const widgetCard = new AIOSWidgetCard(document.getElementById('widgetCard'), {
    size: 'square',
    theme: 'dark',
    title: 'Total Steps',
    value: '5,543'
...
<script>
  const widgetGrid = new AIOSWidgetGrid(document.getElementById('widgetGrid'), {
    dense: true
  });
</script>
```
...
<script>
  const weatherWidget = new AIOSWeatherWidget(document.getElementById('weatherWidget'), {
    temp: 22,
    hi: 26,
    lo: 18,
    city: 'Shanghai',
...
<script>
  const stepsWidget = new AIOSStepsWidget(document.getElementById('stepsWidget'), {
    steps: 5543,
    streak: 5,
    streakUnit: 'days'
  });
...
<script>
  const activityWidget = new AIOSActivityWidget(document.getElementById('activityWidget'), {
    days: [
...
<script>
  const compassWidget = new AIOSCompassWidget(document.getElementById('compassWidget'), {
    heading: 135,
    showDots: true
  });
</script>
...
<script>
  const timeWidget = new AIOSTimeWidget(document.getElementById('timeWidget'), {
    variant: 'over-limit',
    label: 'SCREEN TIME',
    value: '8h 32m',
    unit: 'today',
